...
4. **Friction** — overall distribution (0/1/2/3 counts + %) and average friction score per subcategory, sorted desc.
4a. **Resolution** — counts + % for `Yes`/`Partial`/`No`, and `partial_reason` breakdown (non-null only) with %.
