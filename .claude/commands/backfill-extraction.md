...
3. Before resetting, dump the current values to `scratch/backfill_{YYYY-MM-DD_HHMM}.csv` with columns: `id, intent_action, intent_object, intent_qualifier, qualifier_theme, call_summary, sentiment, resolution, partial_reason, escalation_requested`.
4. Run the reset:
   ```sql
   UPDATE calls SET status='transcribed',
     ...
     resolution=NULL, partial_reason=NULL, escalation_requested=NULL,
     extracted_at=NULL, error_message=NULL
   WHERE <scope>;
   ```
